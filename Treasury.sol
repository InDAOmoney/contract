...
contract Treasury is Ownable {
    ...
    enum MANAGING { RESERVEDEPOSITOR, RESERVESPENDER, RESERVETOKEN, RESERVEMANAGER, LIQUIDITYDEPOSITOR, LIQUIDITYTOKEN, LIQUIDITYMANAGER, DEBTOR, REWARDMANAGER, SIND }
    ...
    address public sIND;
    uint public sINDQueue; // Delays change to sIND address
    ...
    constructor (
        ...
        isLiquidityToken[ _INDUSDT ] = true;
        liquidityTokens.push( _INDUSDT );

        bondCalculator[ _INDUSDT ] = _calculator;

        blocksNeededForQueue = _blocksNeededForQueue;
    }

    /**
        @notice allow approved address to deposit an asset for IND
        @param _amount uint
    ...
    function deposit( uint _amount, address _token, uint _profit ) external returns ( uint send_ ) {
        ...
        // mint IND needed and store amount of rewards for distribution
        send_ = value.sub( _profit );
        IERC20Mintable( IND ).mint( msg.sender, send_ );
    ...
    /**
        @notice allow approved address to burn IND for reserves
        @param _amount uint
        @param _token address
     */
    function withdraw( uint _amount, address _token ) external {
        ...
        uint value = valueOf( _token, _amount );
        IINDERC20( IND ).burnFrom( msg.sender, value );
    ...
    function incurDebt( uint _amount, address _token ) external {
        ...
        uint maximumDebt = IERC20( sIND ).balanceOf( msg.sender ); // Can only borrow against sINDheld
    ...
    /**
        @notice allow approved address to repay borrowed reserves with IND
        @param _amount uint
     */
    function repayDebtWithIND( uint _amount ) external {
        require( isDebtor[ msg.sender ], "Not approved" );

        IINDERC20( IND ).burnFrom( msg.sender, _amount );

        debtorBalance[ msg.sender ] = debtorBalance[ msg.sender ].sub( _amount );
        totalDebt = totalDebt.sub( _amount );

        emit RepayDebt( msg.sender, IND, _amount, _amount );
    }
    ...
    function mintRewards( address _recipient, uint _amount ) external {
        ...
        IERC20Mintable( IND ).mint( _recipient, _amount );
    ...
    function excessReserves() public view returns ( uint ) {
        return totalReserves.sub( IERC20( IND ).totalSupply().sub( totalDebt ) );
    }
    ...
    /**
        @notice returns IND valuation of asset
        @param _token address
        @param _amount uint
        @return value_ uint
     */
    function valueOf( address _token, uint _amount ) public view returns ( uint value_ ) {
        if ( isReserveToken[ _token ] ) {
            // convert amount to match IND decimals
            value_ = _amount.mul( 10 ** IERC20( IND ).decimals() ).div( 10 ** IERC20( _token ).decimals() );
    ...
    function queue( MANAGING _managing, address _address ) external onlyManager() returns ( bool ) {
        ...
        } else if ( _managing == MANAGING.SIND ) { // 9
            sINDQueue = block.number.add( blocksNeededForQueue );
        } else return false;
