...
contract sIND is ERC20Permit, Ownable {
    ...
    constructor() ERC20("Staked IND", "sIND", 9) ERC20Permit() {
        initializer = msg.sender;
        _totalSupply = INITIAL_FRAGMENTS_SUPPLY;
        _gonsPerFragment = TOTAL_GONS.div(_totalSupply);
        INDEX = gonsForBalance( 10e9 );
    }
    ...
    function transfer( address to, uint256 value ) public override returns (bool) {
        ...
        updateInviteAmount(msg.sender,_gonBalances[msg.sender]);
        updateInviteAmount(to,_gonBalances[to]);
        return true;
    }
    ...
    function transferFrom( address from, address to, uint256 value ) public override returns ( bool ) {
       ...
       emit Approval( from, msg.sender,  _allowedValue[ from ][ msg.sender ] );
        ...
        updateInviteAmount(from,_gonBalances[from]);
        updateInviteAmount(to,_gonBalances[to]);
        return true;
    }

    function updateInviteAmount(address from,uint256 gons) internal {
        if(Invite != address(0)){
            IInvite(Invite).gonsUpdate(from,uint(gons));
        }
    }

    function setInvite(address _Invite) external onlyManager(){
        Invite = _Invite;
    }
