...
contract DepositHelper is Ownable {
    ...
    constructor (address _deposit, ISwapV2Router _router, address _LPAddress, address _IND, address _USDT, ITreasury _treasury) {
        deposit = _deposit;
        IND = _IND;
        USDT = _USDT;
    ...
    function depositHelper(
        ...
        uint256 payout = 0;
        if (_tokenAddress == LPAddress) {
            LPAddress.safeTransferFrom(msg.sender, address(this), _amount);
            LPAddress.safeApprove(address(deposit), _amount);
            payout = IDepositor(deposit).deposit(_amount, _maxPrice, msg.sender);
        } else {
            USDT.safeTransferFrom(msg.sender, address(this), _amount);
            USDT.safeApprove(address(router), uint256(- 1));
            IND.safeApprove(address(router), uint256(- 1));
            calAndSwap();
            (,, uint256 moreLPAmount) = router.addLiquidity(USDT, IND, USDT.myBalance(), IND.myBalance(), 0, 0, address(this), block.timestamp);
            LPAddress.safeApprove(address(deposit), moreLPAmount);
            if (USDT.myBalance() > 0) {
                USDT.safeTransfer(msg.sender, USDT.myBalance());
            }
            if (IND.myBalance() > 0) {
                IND.safeTransfer(msg.sender, IND.myBalance());
            }
            IDepositor(deposit).deposit(moreLPAmount, _maxPrice, msg.sender);
        }
    }

    function depositValue(uint256 _amount) public view returns (uint256 value_) {
        ISwapV2Pair lpToken = ISwapV2Pair(factory.getPair(USDT, IND));
        (uint256 token0Reserve, uint256 token1Reserve,) = lpToken.getReserves();
        (uint256 debtReserve, uint256 relativeReserve) = USDT ==
        ...
        if (swapAmt > 0) {
            ...
            (path[0], path[1]) = isReversed ? (IND, USDT) : (USDT, IND);
            uint[] memory amounts = router.getAmountsOut(swapAmt, path);
            (uint256 amount0, uint256 amount1) = USDT == lpToken.token0() ? (_amount.sub(swapAmt), amounts[1]) : (amounts[1], _amount.sub(swapAmt));
            uint256 _totalSupply = lpToken.totalSupply();
            uint256 lpAmount = Math.min(amount0.mul(_totalSupply) / token0Reserve, amount1.mul(_totalSupply) / token1Reserve);
            uint256 value = treasury.valueOf(address(lpToken), lpAmount);
    ...
    function calAndSwap() internal {
        ISwapV2Pair lpToken = ISwapV2Pair(factory.getPair(USDT, IND));
        (uint256 token0Reserve, uint256 token1Reserve,) = lpToken.getReserves();
        (uint256 debtReserve, uint256 relativeReserve) = USDT ==
        lpToken.token0() ? (token0Reserve, token1Reserve) : (token1Reserve, token0Reserve);
        (uint256 swapAmt, bool isReversed) = optimalDeposit(USDT.myBalance(), IND.myBalance(),
            debtReserve, relativeReserve);
        if (swapAmt > 0) {
            address[] memory path = new address[](2);
            (path[0], path[1]) = isReversed ? (IND, USDT) : (USDT, IND);
    ...
    function setBondAddress(address _deposit) external onlyPolicy() {
        deposit = _deposit;
    }
}