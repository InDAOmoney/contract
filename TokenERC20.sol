...
contract IND is ERC20Permit, VaultOwned {
    ...
    constructor() ERC20("Infinity DAO", "IND", 9) {
        whiteListed[msg.sender] = true;
        feeAddress = 0x5Cc30C39f2817c266908c6456295383d4714D8b4;
    }
    ...
    function transfer(address recipient, uint256 amount) public virtual override returns (bool) {
        ...
        if(whiteListed[msg.sender] == false && recipient == pair){
            tAmount = amount.mul(fee).div(feeDenominator);
            _transfer(msg.sender, feeAddress, tAmount);
    ...
    function transferFrom(address sender, address recipient, uint256 amount) public virtual override returns (bool) {
        uint256 tAmount = amount;
        if(whiteListed[sender] == false && recipient == pair){
            tAmount = amount.mul(fee).div(feeDenominator);
            _transfer(sender, feeAddress, tAmount);
            tAmount = amount.sub(tAmount);
        }
